      ...
      <div className="max-w-md mx-auto flex flex-col gap-6">
        ...
        <section className="bg-white/5 p-4 rounded-xl border border-white/10">
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            <Settings size={20} /> Modo de Jogo
          </h2>
          <div className="flex gap-2">
            <button
              onClick={() =>
                setLocalSettings({ ...localSettings, mode: "teams" })
              }
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() =>
                  setLocalSettings((s) => ({
                    ...s,
                    prendasEnabled: !s.prendasEnabled,
                  }))
                }
                className={`w-12 h-6 rounded-full transition-colors relative ${localSettings.prendasEnabled ? "bg-green-500" : "bg-gray-600"}`}
              >
                <div
                  className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${localSettings.prendasEnabled ? "left-7" : "left-1"}`}
                />
              </button>
            </div>
          </div>
        ...
        {/* Teams Section (only show in team mode) */}
        {isTeamMode && (
          <section className="bg-white/5 p-4 rounded-xl border border-white/10">
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
        ...
        {/* Players Section (only show in individual mode) */}
        {isIndividualMode && (
          <section className="bg-white/5 p-4 rounded-xl border border-white/10">
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            ...
            <div className="flex flex-col gap-2">
              ...
              {players.length > 0 && (
                <button
                  onClick={() => useGameStore.setState({ players: [] })}
                  className="text-white/40 text-xs hover:text-red-300 transition-colors w-full text-center mt-2 underline"
                >

  ...
  return (
    ...
    >
        ...
        <div className="w-full flex-1 flex flex-col gap-2">
            <div className="flex items-center gap-2 text-red-500 font-bold uppercase text-sm px-2 border-b-2 border-red-100 pb-2">
                <Ban size={16} /> Palavras Proibidas
            </div>
            {card.forbiddenWords.map((word, idx) => (
                <div key={idx} className="bg-red-50 rounded px-4 py-1.5 font-semibold text-gray-600 border-l-4 border-red-400">
                    {word}
                </div>
            ))}
        </div>

...
export const Scoreboard = () => {
  ...
  if (settings.mode === "individual") {
    ...
    const currentPlayer = players[currentReaderIndex];
        ...
        <div className="flex items-center gap-2 bg-black/20 backdrop-blur-sm p-1 sm:p-2 rounded-full border border-white/10 justify-center max-w-full overflow-x-auto">
          {sortedScores.map((player) => {
            ...
            return (
              ...
              >
                {/* Rank Indicator */}
                <div className="w-2 sm:w-3 h-2 sm:h-3 rounded-full shadow-[0_0_10px_rgba(0,0,0,0.5)] flex-shrink-0 bg-yellow-400" />

                {/* Name + Score */}
                <div className="flex flex-col items-start min-w-0">
                  <span
                    className={`text-[8px] sm:text-[10px] uppercase font-bold tracking-wider leading-none truncate ${isCurrent ? "text-white" : "text-white/70"}`}
                  >
                    {player.playerName}
                  </span>
                  <span
                    className={`text-xl sm:text-2xl font-black leading-none truncate ${isCurrent ? "text-white" : "text-white/60"}`}
                  >
                    {player.score}
                  </span>
                </div>
